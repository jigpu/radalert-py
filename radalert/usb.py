"""
USB protocol implmentation for Radiation Alert geiger counters.

The recent Radiation Alert series of geiger counters from SE International
provide a USB connection that can be used to communicate with the device.
This module implements the protocol used by this connection, decoding the
packets into a form which is more easily usable.
...
class RadAlertUSBStatus(generic.RadAlertStatus):
    """
    Representation of a status packet from a RadAlertUSB device.
    ...
    def display_value(self) -> float:
        ...
        info = RadAlertUSBStatus._MODE_DISPLAY_INFO[mode]
        return info[1](value)
    ...
    def unpack(bytestr: bytes) -> Dict[str, Union[int, bool]]:
        ...
        RadAlertUSBStatus._validate(data)
        return data
...
class RadAlertUSBQuery(generic.RadAlertQuery):
    """
    Representation of a query packet from a RadAlertUSB device.
    ...
    def unpack(bytestr: bytes) -> Dict[str, Union[str, int, bool]]:
        ...
        RadAlertUSBQuery._validate(data)
        return data
...
class RadAlertUSB(generic.RadAlert):
    ...
    _HID_IDLE_RATE_INFINITE = 0
    _DEVICE_CONFIGURATION = 0
    _DEVICE_INTERFACE = 0
    _DEVICE_ENDPOINT = 1
    _DEVICE_REPORT_ID_QUERY = 0
    _DEVICE_REPORT_ID_DATA = 1
    _DEVICE_TIMEOUT = 100
    ...
    def connect(self, vid: int, pid: int) -> None:
        ...
        self._usbdev = context.openByVendorIDAndProductID(vid, pid)
        if self._usbdev is None:
            raise OSError("Unable to open USB device")
        self._usbdev.setAutoDetachKernelDriver(True)
        self._usbdev.claimInterface(RadAlertUSB._DEVICE_INTERFACE)
        self._reset()
    ...
    def disconnect(self) -> None:
        ...
        if self._usbdev is not None:
            try:
                self._usbdev.releaseInterface(RadAlertUSB._DEVICE_INTERFACE)
            except:
                pass
            self._usbdev.close()
            self._usbdev = None
    ...
    def trigger_query(self) -> RadAlertUSBQuery:
        """
